...
var publicLib = {
    ...
    toast_err: function(flag, title) {
        if (!flag || flag == 'show') {
            $("#fullscreen-overlay1").fadeIn(100);
            $("#toast_error").fadeIn(100);
            $('#toast_error_cancel').removeClass('hidden')
        } else if (flag == 'hide') {
            $("#toast_error").fadeOut(100);
            $("#fullscreen-overlay1").fadeOut(100);
            $('#toast_error_cancel').addClass('hidden')

        };
        if (title) {
            $("#toast_error_title").html(title);
        } else {
            $("#toast_error_title").html('无法加载页面啦，检查您的因特网链接。或者稍等一会再次访问。');
        };
    },
